...
export default function Home() {
  ...
  // stage の判定ロジック
  const stage: Stage =
    status && (status.includes('完了!') || status.includes('エラー:'))
      ? 'idle'
      : status?.includes('録音中')
      ? 'recording'
      : status?.includes('推論中…') || status?.includes('WAV化中…')
      ? 'inference'
      : 'idle';
  ...
  // 操作パネル表示完了後（アニメーション完了後）に免責事項を出す
  useEffect(() => {
    // Intro 2.8s + delay 3s + duration 1.5s ≒ 合計 4.3～5秒後に表示
    const timer = setTimeout(() => setDisclaimerOpen(true), 5000);
    return () => clearTimeout(timer);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br">
      ...
      <motion.div
        ...
        animate={{ opacity: 1, scale: 1, y: ['0%', '0%', '0%'] }}
        transition={{ duration: 1.5, delay: 3 }}
      >
        <main className="relative z-10 min-h-screen flex items-center justify-center px-2 sm:px-4 py-8">
          {/* 操作パネル・キャラクター */}
          <div className="relative w-full max-w-xs sm:max-w-md md:max-w-3xl lg:max-w-4xl bg-black/70 backdrop-blur-xl rounded-2xl p-4 sm:p-6 md:p-8 border border-green-500/60 shadow-2xl shadow-green-500/50 flex flex-col md:flex-row gap-4 md:gap-6 lg:gap-8 items-stretch">
            {/* Controls (操作ボタン類) */}
